...
from models.misc import ResLayer, LogDropoutM
...
class BLHmmLm(nn.Module):
    def __init__(self, num_notes, config):
        ...
        self.scalar_emit = config.scalar_emit
        ...
        if self.scalar_emit:
            self.emission_logits = nn.Parameter(
                th.randn(self.C, 88, 2),
            )
        else:
            # p(xt | zt)
        ...
        if self.keep_counts:
            self.register_buffer(
                "counts",
                th.zeros(self.C, self.num_notes),
            )
            self.register_buffer(
                "state_counts",
    ...
    def emission(self, mask=None):
        ...
        if self.config.emission_topk:
            ...
            mask = th.zeros(self.C, self.num_notes, device=self.device, dtype=th.bool)
        ...
        # shortcut out to scalar emission
        if self.scalar_emit:
            return self.emission_logits.log_softmax(-1)
    ...
    def compute_loss(                                           
        ...
        p_emit = emission[
            ...
            th.arange(num_notes)[None,None,None],
            text[:,:,None],
        ].sum(-1)
    ...
    def score(self, text, lpz=None, last_states=None, mask=None, lengths=None):
        ...
        if not self.config.emission_dropout:
            ...
            emission_mask = (th.empty(num_states, self.num_notes, device=self.device)
        ...
        p_emit = emission[
            th.arange(num_states)[None,None,:,None],
