...
class SemiMarkovCRF(StructDistribution):
    ...
    * Forward Memory: :math:`O(N \log(N) C^2 K^2)`
    ...
    """
    def __init__(self, log_potentials, lengths=None, args={}):
        scores_left, scores_right, scores_1_C, scores_C_1, scores_1_1 = log_potentials
        b = scores_left.size(0)
        N = scores_left.size(1)
        K = scores_left.size(2)
        C = scores_left.size(3) + 1
        scores = torch.zeros(b, N, K, C, C)

        batch_shape = scores.shape[:1]
        event_shape = scores.shape[1:]
        #self.log_potentials = log_potentials
        self.log_potentials = log_potentials
        self.lengths = lengths
        self.args = args
