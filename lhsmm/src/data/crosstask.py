...
class CrosstaskDatasplit(Datasplit):
    def __init__(self, corpus, remove_background, task_sets=None,
        ...
        if not self.full:
            ...
            #}
            _video_names_by_task = {}
            for task_index in sorted(self._video_names_by_task.keys()):
                import random
                random.seed(1234)
                videos = list(self._video_names_by_task[task_index])
                random.shuffle(videos)
                _video_names_by_task[task_index] = videos[:int(math.floor(0.1*len(videos)))]
                #task_index: videos[:10]
                #for task_index, videos in list(self._video_names_by_task.items())
            #}
            self._video_names_by_task = _video_names_by_task
        else:
            if 'train' in split:
                _video_names_by_task = {}
                for task_index in sorted(self._video_names_by_task.keys()):
...
class CrosstaskGroundTruth(GroundTruth):
    ...
    def _load_gt(self):
        ...
        def get_T(filename):
            ...
            if video not in self._t_by_video:
                #import pdb; pdb.set_trace()
                print (video)
        ...
        if self._corpus.load_constraints:
            ...
            for constraint_fname in filenames:
                file = os.path.split(constraint_fname)[1]
                file_no_ext = os.path.splitext(file)[0]
                splits = file_no_ext.split('_')
                task = int(splits[0])
                if task not in self._task_names:
